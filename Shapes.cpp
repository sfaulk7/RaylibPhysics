...
glm::vec2 DepenetrateAABBAABB(const glm::vec2& PosA, const AABB& AABBA, const glm::vec2& PosB, const AABB& AABBB, float& Pen)
{
    ...
    float closestPointOnAABBX = glm::clamp(PosB.x, PosA.x - glm::abs(AABBA.HalfExtents.x), PosA.x + glm::abs(AABBA.HalfExtents.x));
    float closestPointOnAABBY = glm::clamp(PosB.y, PosA.y - glm::abs(AABBA.HalfExtents.y), PosA.y + glm::abs(AABBA.HalfExtents.y));
    glm::vec2 closestPointOnAABB = glm::vec2(closestPointOnAABBX, closestPointOnAABBY);

    return glm::normalize(PosB - closestPointOnAABB);
}
...
glm::vec2 DepenetrateCircleAABB(const glm::vec2& PosA, const Circle& CircleA, const glm::vec2& PosB, const AABB& AABBB, float& Pen)
{
    ...
    float penXA = 10;
    float penXB = 10;
    float closestPointOnAABBBX = glm::clamp(PosA.x, PosB.x - glm::abs(AABBB.HalfExtents.x), PosB.x + glm::abs(AABBB.HalfExtents.x));
    float closestPointOnAABBBY = glm::clamp(PosA.y, PosB.y - glm::abs(AABBB.HalfExtents.y), PosB.y + glm::abs(AABBB.HalfExtents.y));
    glm::vec2 closestPointOnAABBB = glm::vec2(closestPointOnAABBBX, closestPointOnAABBBY);

    float closestPointDistance = glm::distance(PosB, closestPointOnAABBB);
    float circleDistance = glm::distance(PosA, PosB);
    ...
    {
        //
        penYA = (PosA.y + CircleA.Radius) - closestPointOnAABBB.y;
    }
    else
    {
        //
        penYB = (PosA.y - CircleA.Radius) - closestPointOnAABBB.y;
    }
    if (ALeftOfB == true)
    {
        //
        penXA = (PosA.x + CircleA.Radius) - closestPointOnAABBB.x;
    }
    else
    {
        //
        penXB = (PosA.x - CircleA.Radius) - closestPointOnAABBB.x;
    }
    ...
    return glm::normalize(closestPointOnAABBB - PosA);
}
